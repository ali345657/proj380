	...
	public class Proj380{
		...
		public static void main(String[] args) {
			EventQueue.invokeLater(new Runnable() {
				public void run() {
					try {
						Proj380 Proj380 = new Proj380();
						Proj380.frame.setVisible(true);
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
		...
		public void connect(){
		 ...
		 private JTextField txtSsnSearch;
		...
		public void table_load() {
			try {
				pst = con.prepareStatement("SELECT * FROM `coffe`.`order-`");
		...
		public void load_data() {
			...
			try {
	            ...
	            String sqlOrder="SELECT * FROM `coffe`.`order-`";
		...
		private void initialize() {
			...
			panel_4.setBounds(728, 437, 312, 283);
			...
			btnNewButton_2.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					try {
						pst = con.prepareStatement("INSERT INTO `coffe`.`prepare`(`order_Ordernum`,`employee_ssn`) VALUES(?, ?) ");
			...
			btnNewButton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					try {
						pst = con.prepareStatement("INSERT INTO `coffe`.`order-`(`Ordernum`,`Ordertime`,`Item`,`amount`,`customer_customer_phone`) VALUES(?, ?, ?, ?, ?) ");
						pst.setString(1, Ordernum);
			...
			btnUpdate.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					try {
						pst = con.prepareStatement("UPDATE `coffe`.`order-` SET `Ordernum` = ?, `Ordertime` = ?, `Item` = ?, `amount` = ?, `customer_customer_phone` = ? WHERE `Ordernum` = ? ");
						pst.setString(1, Ordernum);
						pst.setString(2, Ordertime);
						pst.setString(3, Item);
						pst.setString(4, amount);
			...
			btnDelete.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					try {
						pst = con.prepareStatement("DELETE FROM `coffe`.`order-`WHERE Ordernum =?");
						pst.setString(1, Ordernum);
						pst.executeUpdate();
			...
			panel_1.setBounds(728, 34, 312, 59);
			...
			txtPhone.addKeyListener(new KeyAdapter() {
				...
				public void keyReleased(KeyEvent e) {
					try {
						...
						pst =con.prepareStatement("select name,age,sex from customer where customer_phone=? ");
						pst.setString(1, id);
			...
			panel_2.setBounds(728, 104, 312, 283);
			...
			panel_3.setBounds(395, 100, 290, 297);
			...
			panel_5.setBounds(1097, 437, 281, 283);
			frame.getContentPane().add(panel_5);
			panel_5.setLayout(null);
			
			tblPrepare = new JTable();
			tblPrepare.setBounds(10, 11, 261, 261);
			...
			panel_1_1.setBounds(1097, 361, 312, 59);
			frame.getContentPane().add(panel_1_1);
			
			JLabel lblNewLabel_1_1_1_1 = new JLabel("employee ssn");
			lblNewLabel_1_1_1_1.setFont(new Font("Tahoma", Font.PLAIN, 15));
			lblNewLabel_1_1_1_1.setBounds(30, 20, 117, 19);
			panel_1_1.add(lblNewLabel_1_1_1_1);
			
			txtSsnSearch = new JTextField();
			txtSsnSearch.addKeyListener(new KeyAdapter() {
				@Override
				public void keyReleased(KeyEvent e) {
					try {
						pst = con.prepareStatement("SELECT * FROM `coffe`.`prepare` WHERE employee_ssn = ?");
						pst.setString(1, txtSsnSearch.getText());
						rs = pst.executeQuery();
			...
			txtSsnSearch.setColumns(10);
			txtSsnSearch.setBounds(157, 21, 86, 20);
			panel_1_1.add(txtSsnSearch);
			
			
		}
		public void table_prepare_load() {
			try {
				pst = con.prepareStatement("SELECT * FROM `coffe`.`prepare`");
